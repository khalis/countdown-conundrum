...
def generate_cpp(templates):
  expr_str = ', 0b'.join(templates)
  print("array all_expressions = to_array<size_t>({0b%s});" % expr_str)
  print("size_t func(int a, int b, int c, int d, int e, int f, op op1, op op2, op op3, op op4, op op5, int goal){\n\tsize_t res = 0;")
    ...
    for char in expr[::-1]:
      match(char):
        ...
        case '0':
          ...
          op_number += 1
    print(f'\tres |= static_cast<size_t>(goal == {stack[-1]}) << {ind}; // {expr}')
  print("\treturn res;\n}")
...
generate_cpp(generate_templates())
